...
public class TownGraphManager_STUDENT_Test {
	...
	public void testAddRoad() {
		...
		assertEquals("Road_2", roads.get(0));
		assertEquals("Road_5", roads.get(1));
		assertEquals("Road_11", roads.get(2));
		assertEquals("Road_6", roads.get(3));
		graph.addRoad(town[4], town[11], 1,"Road_13");
		roads = graph.allRoads();
		assertEquals("Road_2", roads.get(0));
		assertEquals("Road_10", roads.get(1));
		assertEquals("Road_12", roads.get(2));
		assertEquals("Road_11", roads.get(3));
		assertEquals("Road_9", roads.get(4));
		
	}

	@Test
	public void testGetRoad() {
		assertEquals("Road_14", graph.getRoad(town[2], town[11]));
		assertEquals("Road_6", graph.getRoad(town[3], town[7]));
	}
	...
	public void testAllRoads() {
		ArrayList<String> roads = graph.allRoads();
		assertEquals("Road_2", roads.get(0));
		assertEquals("Road_12", roads.get(1));
		assertEquals("Road_11", roads.get(2));
	...
	public void testDeleteRoadConnection() {
		assertEquals(true, graph.containsRoadConnection(town[2], town[11]));
		graph.deleteRoadConnection(town[2], town[11], "Road_10");
	...
	public void testAllTowns() {
		...
		assertEquals("Town_2", roads.get(0));
		assertEquals("Town_5", roads.get(1));
		assertEquals("Town_3", roads.get(2));
		assertEquals("Town_6", roads.get(3));
	...
	public void testGetPath() {
		ArrayList<String> path = graph.getPath(town[1],town[11]);
		  ...
		  assertEquals("Town_2 via Road_12 to Town_11 4 mi",path.get(1).trim());
	...
	public void testGetPathA() {
		ArrayList<String> path = graph.getPath(town[1],town[10]);
		  assertNotNull(path);
		  assertTrue(path.size() > 0);
		  assertEquals("Town_1 via Road_2 to Town_3 5 mi",path.get(0).trim());
		  assertEquals("Town_3 via Road_5 to Town_8 7 mi",path.get(1).trim());
		  assertEquals("Town_8 via Road_9 to Town_10 8 mi",path.get(2).trim());
	}
	...
	public void testGetPathB() {
		ArrayList<String> path = graph.getPath(town[1],town[6]);
		  ...
		  assertEquals("Town_10 via Road_8 to Town_9 6 mi",path.get(3).trim());
